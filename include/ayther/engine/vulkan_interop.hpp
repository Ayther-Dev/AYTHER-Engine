...
struct VulkanContextView {
    ...
    VmaAllocator allocator_handle{nullptr};
    ...
    [[nodiscard]] constexpr VmaAllocator allocator() const noexcept {
...
/// extend their lifetime and a consumer must never destroy or free them.
///
...
/// Queue-family transfer is not part of this contract and requires an
/// additional API that explicitly coordinates ownership in both directions.
///
...
///
/// Views returned by AytherRenderer::render_image() remain valid until that
/// renderer is resized, shut down, or destroyed. Releasing or recapturing the
/// comparison image invalidates every previously returned comparison view, even
/// when a recapture reuses the same Vulkan handles. Consumers must finish all
/// GPU use and discard or rebind descriptors before any such operation.
/// Destroying a RenderImageView value has no Vulkan effect.
