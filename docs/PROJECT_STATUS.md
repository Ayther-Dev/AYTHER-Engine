...
primitives are present. CodeQL analysis runs on `main`, but one critical and one
high finding are open. Of the eight release blockers, three are closed, three
remain open, and two are deferred on components that live outside this repository.
...
The complete protected CI matrix passed on commit `d68cfad` in
[run 33461455942](https://github.com/Ayther-Dev/AYTHER-Engine/actions/runs/33461455942):
...
as a passing hardware result. The CodeQL workflow completed, but
[one critical](https://github.com/Ayther-Dev/AYTHER-Engine/security/code-scanning/2)
and [one high](https://github.com/Ayther-Dev/AYTHER-Engine/security/code-scanning/1)
finding remain open on that same commit.

The `main` ruleset currently requires a pull request and strict CI but no
approving or Code Owner review. This is recorded as the temporary
[single-maintainer exception](GOVERNANCE_EXCEPTIONS.md#gov-2026-001-single-maintainer-code-owner-review):
it permits maintenance while the eligible team has one member, but it is not
independent review and cannot be presented as two-person release approval.
...
| CI and release workflows | Reproducible pre-release path | Tag releases build the four official Engine archives, attest them, and gate protected publication on a fresh Windows/Linux × standard/VPX matrix that verifies the packaged bytes, builds the pinned external Runtime, runs its CTests, and rejects producer paths in caches/imported targets |
...
| 7 | Complete security review of media-decoder limits, scripting lifetimes, FFI ownership, dynamic library loading, and adversarial fuzz coverage | Open, confirmed defect repaired; new findings open | Substantial parts are built: decoded-resource ceilings for image, audio, and video, a 64 MiB Lua memory cap, ASan and UBSan jobs, and three fuzz targets. The 43-byte SoundFont reproducer that previously requested 4 GiB is now rejected by `validated_sf2_extent()` before the third-party parser; the regression covers both `Sf2Synth::new()` and `new_shared()`. ASan, UBSan, and all three seeded fuzz targets passed in [run 33461455942](https://github.com/Ayther-Dev/AYTHER-Engine/actions/runs/33461455942). CodeQL now reports [one critical](https://github.com/Ayther-Dev/AYTHER-Engine/security/code-scanning/2) and [one high](https://github.com/Ayther-Dev/AYTHER-Engine/security/code-scanning/1) finding on `main`, and the wider security review remains open. See the [current go/no-go decision](RELEASE_GO_NO_GO_CURRENT.md) |
...
Of those six requirements, three are met and three are not:
...
| Private vulnerability reporting | **Yes** | GitHub private vulnerability reporting is enabled and [SECURITY.md](../SECURITY.md) documents the channel |
...
The stability gate was executed against remote `main` commit `d68cfad` on
2026-08-31 and returned **no-go**; the evidence for each criterion is recorded
in the [current go/no-go decision](RELEASE_GO_NO_GO_CURRENT.md). The
[2026-08-30 decision](RELEASE_GO_NO_GO.md) remains a historical snapshot. The
remaining distance to “stable” is therefore both the blocker list and parts of
the release machinery that are implemented but have never been operated.
